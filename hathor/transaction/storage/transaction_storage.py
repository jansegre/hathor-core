...
from typing import Any, Dict, FrozenSet, Generator, Iterable, Iterator, List, NamedTuple, Optional, Set, Tuple, cast
...
from hathor.util import not_none, skip_warning
...
class TransactionStorage(ABC):
    ...
    def del_from_deps_index(self, tx: bytes) -> None:
        """Call to remove tx from all a reverse dependencies, for example when validation is complete."""
    ...
    def add_needed_deps(self, tx: BaseTransaction) -> None:
        ...
        # self.needed_txs.update((i, (height, not_none(tx.hash))) for i in tx.get_tx_parents())
    ...
    def add_to_block_height_index(self, height: int, block_hash: bytes) -> bool:
        return self._block_height_index.add(height, block_hash)
...
class TransactionStorageAsyncFromSync(TransactionStorage):
    ...
    def save_transaction_deferred(self, tx: BaseTransaction, *, only_metadata: bool = False,
                                  add_to_indexes: bool = False) -> Deferred:
